...
const regionalLinkSchema = z.object({
    customer_id: z.any(),
    startDate: z.any(),
    endDate: z.any(),
    sla_id: z.any(),
...
type FormData = z.infer<typeof regionalLinkSchema >


function RegionalLinkReport() {
    ...
    const { isLoading: slaLoading, error: slaError, data:slaData } = useSingleCustomer('/ticketing', parseInt(selectedCustomerId || "0"));
    const slas = Array.isArray(slaData) ? slaData.reverse() : [];
    ...
    const form = useForm<FormData>({
        resolver: zodResolver(regionalLinkSchema),
        defaultValues: {
            customer_id: 0,
    ...
    async function onSubmit(data: FormData) {
        try {
            ...
            const url = `https://www.repository.qtask.net/fpdf/generate/link_reports.php?${queryParams}`;
            ...
            link.href = blobUrl;
            link.setAttribute('download', 'Regional_Link_Report.pdf'); // Set the file name
            ...
            if (response.status === 200) {
                ...
                toast({
                    ...
                    description: 'You have successfully generated regional link report.',
                    variant: 'default',
                });
...
export default RegionalLinkReport;