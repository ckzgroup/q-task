...
function SegmentReport() {
  ...
  const { isLoading:segLoadiing, error:segError, data:segData } = useSingleSegment('/project_segment/getSegment', segment_id)
  const segment = Array.isArray(segData) ? segData.reverse() : [];
  const {
    segment_name
  } = segment[0] || {};
  ...
  async function onSubmit(data: FormData) {
        try {
            ...
            const url = `https://www.repository.qtask.net/fpdf/generate/segment_report.php?${queryParams}`;
            ...
            link.href = blobUrl;
            link.setAttribute('download', `${segment_name}_segment_report.pdf`); // Set the file name
            ...
            if (response.status === 200) {
                ...
                toast({
                    ...
                    description: 'You have successfully generated annual segment report.',
                    variant: 'default',
                });
        ...
        <Form {...form}>
            ...
            {/* Filters */}
                <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                    <Button type="submit" className="space-x-2 mt-2">
                        <span> Generate Segment Report </span>
                    </Button>
